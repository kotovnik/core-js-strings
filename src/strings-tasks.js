...
function removeFirstOccurrences(str, value) {
  ...
  const firstString = str.substring(0, firstIndex);
  const secondString = str.substring(secondIndex);
  const resultString = firstString.concat(secondString);
  return resultString;
}
...
function removeLastOccurrences(str, value) {
  const secondIndex = str.lastIndexOf(value) + value.length;
  if (str.lastIndexOf(value) < 0) {
    return str;
  }
  const firstIndex = secondIndex - value.length;
  const firstString = str.substring(0, firstIndex);
  const secondString = str.substring(secondIndex);
  const resultString = firstString.concat(secondString);
  return resultString;
...
function formatTime(minutes, seconds) {
  ...
  const resultTime = `${minutesTime}:${secondsTime}`;
  return resultTime;
}
...
function countVowels(str) {
  let count = 0;
  const stringOfVowels = 'aeiouyAEIOUY';
  for (let i = 0; i < str.length; i += 1) {
    for (let y = 0; y < stringOfVowels.length; y += 1) {
      if (str[i] === stringOfVowels[y]) {
        count += 1;
      }
    }
  }
...
function isPalindrome(str) {
  ...
  const letterFromEnd = resultString.length - 1;
  for (let i = 0; i < resultString.length; i += 1) {
    if (resultString[i] !== resultString[letterFromEnd - i]) {
      result = false;
      break;
    }
    result = true;
...
function findLongestWord(sentence) {
  ...
  const arraySorted = arr.reduce((bigger, smaller) =>
    bigger.length >= smaller.length ? bigger : smaller
  );
  return arraySorted;
}
...
function extractNameFromTemplate(value) {
  ...
  for (let i = 0; i < value.length; i += 1) {
    if (value[i] === ' ') {
      resultString = value.substring(i + 1);
      break;
    }
  }
  const result = resultString.substring(0, resultString.length - 1);
  return result;
}
...
function encodeToRot13(str) {
  let result = '';
  const standartString = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const codeString = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  for (let i = 0; i < str.length; i += 1) {
    for (let y = 0; y < standartString.length; y += 1) {
      if (str[i] === standartString[y]) {
        result += codeString[y];
        break;
      }
    }
